...
unsigned short lcd_read_panelid(void)
{
	...
	idio_config.direction = kGPIO_DigitialOutput; 
	...
	idio_config.direction = kGPIO_DigitialInput;
	...
	if(idx==0)return ATK4342;       //4.3 480*272
	else if(idx==1)return ATK7084;	//7 800*480
	else if(idx==2)return ATK7016;	//7 1024*600
	else if(idx==4)return ATK4384;	//4 800*480
	else if(idx==5)return ATK1018;	//10.1 1280*800		
	else return 0;
}
...
void lcdgpio_init(void)
{
    ...
    gpio_config.direction = kGPIO_DigitialOutput; 
...
inline unsigned lcd_readpoint(unsigned short x, unsigned short y)
{
